...
- Word 内容：只验收图产物登记的单一 canonical DOCX，不用目录 glob 选择“最新”文件；唯一 Title/Codex Book Title 段、核心属性书名和 EPUB 书名一致；允许与核心作者一致的居中作者段；每章 reader-visible 全文与 Markdown 一致；Heading 1–3 签名精确相等（源 H4–H6 按生成规则折叠为 Heading 3）；表格形状、每条引文的原文/归属/顺序、粗体/斜体/下划线片段均精确对应。分页只允许扉页后的单一空分页段和一级章节标题的 `pageBreakBefore`，其他显式分页仍阻断；成品不得包含页眉、页脚、`PAGE` 或 `NUMPAGES` 字段。
