...
def _docx_expectations(
    ...
    for item in selected:
        ...
        source = chapter_texts.get(chapter_id, "")
...
def _check_citations(context: _VerificationContext) -> dict[str, Any]:
    ...
    for item in context.selected:
        ...
        strict = bool(item.get("reviewed_override"))
...
def _check_docx(context: _VerificationContext) -> dict[str, Any]:
    ...
    try:
        with zipfile.ZipFile(path) as archive:
            ...
            try:
                ...
                document_root = None
            if re.search(
                r"<w:(?:br\b[^>]*w:type=\"page\"|lastRenderedPageBreak\b|pageBreakBefore\b)",
                document_xml,
            ):
                issues.append(
                    _issue(
    ...
    try:
        ...
        title_paragraphs = [
            paragraph.text.strip()
            for paragraph in document.paragraphs
            if getattr(paragraph.style, "name", "") == "Title"
        ]
        ...
        expected_preamble = [("Title", inferred_title)] if inferred_title else []
...
def verify_publication(
    ...
    unexpected_skips = bool(
        ...
        and any(check["status"] == "skipped" for check in checks)
    )
